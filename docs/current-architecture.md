...
- External job-provider API connector.
...
- Passwords are hashed with unsalted SHA-256 in `backend/app/security.py`.
...
- Authenticated requests can also use `X-User-Id`, which is retained as a local/test fallback.
...
- Password hashing is not production-grade because it uses fast unsalted SHA-256 rather than Argon2, bcrypt, or scrypt.
- `X-User-Id` fallback can bypass password/token possession if exposed in production.
...
- `X-User-Id` fallback can authenticate requests without possession of a password or bearer token if exposed outside local/test environments.
